...
namespace ht {
    ...
    namespace detail {
        ...
        template <class From, class To, class ...Tail> struct first_convertible_to<From, To, Tail...>: std::conditional_t<std::is_convertible_v<From, To>, std::type_identity<To>, first_convertible_to<From, Tail...>>{ };
...
#pragma ide diagnostic ignored "misc-unconventional-assign-operator"
        template <class U> requires (std::convertible_to<U, T> or ...) constexpr auto operator=(U&& u) const {
...
#pragma clang diagnostic pop
    ...
    template <class AttrsTuple, class ChildrenTuple> struct element_aspects {
        ...
        constexpr element_aspects(AttrsTuple&& _set_attrs, ChildrenTuple&& _children): set_attrs{ std::move(_set_attrs) }, children{ std::move(_children) } { }
    };
    ...
    namespace detail {
        ...
        template <template <class> class Predicate, class T, class ...Tail> struct filter_types<Predicate, T, Tail...> {
            using type = typename std::conditional<Predicate<T>::value, decltype(std::tuple_cat(std::declval<std::tuple<T>>(), std::declval<typename filter_types<Predicate, Tail...>::type>())), typename filter_types<Predicate, Tail...>::type>::type;
        };
        ...
        template <static_string AttrName, class AttrType, static_string In> struct is_attribute_ptr<attribute<AttrName, AttrType> attribute_list<In>::*>: std::true_type { };
        ...
        template <static_string AttrName, class A, class ...Tail> struct find_attr_holder<AttrName, A, Tail...>: find_attr_holder<AttrName, Tail...> { };
        template <static_string AttrName, static_string In, class AttrType, class ...Tail> struct find_attr_holder<AttrName, attribute<AttrName, AttrType> attribute_list<In>::*, Tail...>: std::true_type {
        ...
        template <static_string AttrName, class AttrType, class ...AttrPtr> struct find_attr_holder_in_tuple<attribute<AttrName, AttrType>, std::tuple<AttrPtr...>>: find_attr_holder<AttrName, AttrPtr...> { };

        template <auto A> struct interface_name_from_ptr { };
        template <static_string In, static_string AttrName, class AttrType, attribute<AttrName, AttrType> attribute_list<In>::* Ptr> struct interface_name_from_ptr<Ptr> {
        ...
        template <static_string AttrName, class ...AttrPtrs> constexpr bool attributes_contain_v = find_attr_holder<AttrName, AttrPtrs...>::value;
        
        template <static_string El, class ...AttrPtrs, class ...SetAttrs, class ...Children> constexpr auto initialize_attribute(attribute_list<El>& t, std::tuple<AttrPtrs...> ptrs, std::tuple<SetAttrs...>, std::tuple<Children...> children) {
            return element_aspects<std::tuple<>, std::tuple<>>{ };
        }

        template <static_string El, class ...AttrPtrs, class V, class ...R, class ...SetAttrs, class ...Children> constexpr auto initialize_attribute(attribute_list<El>& t, std::tuple<AttrPtrs...> ptrs, std::tuple<SetAttrs...> set_attrs, std::tuple<Children...> children, V&& value, R&& ...r) {
            ...
            static constexpr auto take_attr = [](auto& t, auto& ptrs, auto&& set_attrs, auto&& value){
                if constexpr (attr_name::value) {
                    static_assert(attributes_contain_v<attr_name::name, AttrPtrs...>, "attribute does not apply to the specified element");
            ...
            if constexpr (sizeof...(R) == 0) {
                return element_aspects{ std::move(take_attr(t, ptrs, set_attrs, value)), std::move(take_child(t, children, value)) };
            } else {
                return initialize_attribute(t, std::move(ptrs), take_attr(t, ptrs, set_attrs, value), take_child(t, children, value), std::forward<R>(r)...);
            }
        }
    }
}
