...
namespace ht {
    ...
    struct braided_renderer {
        template <std::size_t Depth = 0, std::convertible_to<std::string_view> T> constexpr sv_braid auto render(const T& sv, const render_decoration&& dec = { }) const {
            return braid { std::move(dec.pre), std::move(static_cast<std::string_view>(sv)) } + std::move(dec.post);
        }

        template <std::size_t Depth = 0, class T> requires (sv_braid<std::invoke_result_t<custom_renderer<T>, const T&, const render_decoration&&>>) constexpr sv_braid auto render(const T& t, const render_decoration&& dec = { }) const {
            return std::move(custom_renderer<T>{ }(t, std::move(dec)));
        }

        template <std::size_t Depth = 0, static_string AttrName, class AttrType> constexpr sv_braid auto render(const attribute<AttrName, AttrType>& attr, const render_decoration&& dec = { }) const {            
        ...
        template <std::size_t Depth = 0, static_string Tag, static_string In, class ...Parents, auto ...Attrs, class ...Children> constexpr sv_braid auto render(const element<Tag, interface_spec<In, std::tuple<Parents...>, Attrs...>, std::tuple<Children...>>& el, const render_decoration&& dec = { }) const {
            using element_type = const element<Tag, interface_spec<In, std::tuple<Parents...>, Attrs...>, std::tuple<Children...>>; 
            ...
            if constexpr (std::tuple_size_v<typename element_type::children_tuple> == 0) {
            ...
            } else if constexpr (std::tuple_size_v<typename element_type::children_tuple> == 1 && std::is_convertible_v<std::tuple_element_t<0, typename element_type::children_tuple>, std::string_view>) {
                ...
                return std::move(dec.pre) + std::move(tag_in) + std::move(ch) + indent<Depth>() + std::move(tag_out) + std::move(dec.post);
            }
        }
...
#define HT_HTML_BRAID_LITERAL(element) HT_BRAID_LITERAL(::ht::braided_renderer::render_element(element))
