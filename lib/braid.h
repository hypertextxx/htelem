...
namespace ht {
    ...
    template <class T> struct braid_common_type: std::conditional_t<!detail::is_braid_v<T>, std::type_identity<T>, std::type_identity_t<empty_strand>> { };
    template <class L> struct braid_common_type<braid<L, empty_strand>>: std::type_identity<L>{ };
    template <class L, class R> struct braid_common_type<braid<L, R>>: std::common_type<std::conditional_t<detail::is_braid_v<L>, typename braid_common_type<std::decay_t<L>>::type, std::decay_t<L>>, std::conditional_t<detail::is_braid_v<R>, typename braid_common_type<std::decay_t<R>>::type, std::decay_t<R>>>{ };

    template <class T, class = void> constexpr bool is_uniform_braid_v = false;
    template <class L, class R> constexpr bool is_uniform_braid_v<braid<L, R>, std::void_t<typename braid_common_type<braid<L, R>>::type>> = true;
    ...
    template <class L, class R = empty_strand> struct braid {
        ...
        constexpr auto as_tuple() {
            return reduce([]<class ...T>(std::tuple<T...>&& t, const auto& value){ return std::tuple_cat(std::move(t), std::make_tuple(value)); }, std::make_tuple());
        }

        static constexpr auto count() {
            return detail::count_braid<braid<L, R>>::value;
        }
    };

    template <class L, class R1, class R2> requires (!detail::is_braid_v<R2>) constexpr auto operator+(braid<L, R1>&& b, R2&& rhs) {
        return braid{ std::move(b.left), braid{ std::move(b.right), std::move(rhs) } };
    }

    template <class L, class R> requires (!detail::is_braid_v<R>) constexpr auto operator+(braid<L, empty_strand>&& b, R&& rhs) {
        return braid{ std::move(b.left), std::move(rhs) };
    }

    template <class L1, class L2, class R> requires (!detail::is_braid_v<L1>) constexpr auto operator+(L1&& lhs, braid<L2, R>&& b) {
        return braid{ std::move(lhs), std::move(b) };
    }

    template <class L1, class L2> requires (!detail::is_braid_v<L1>) constexpr auto operator+(L1&& lhs, braid<L2, empty_strand>&& b) {
    ...
    template <class L, class R> requires (is_uniform_braid_v<braid<L, R>>) constexpr auto braid_to_array(const braid<L, R>& br){
        using common_t = braid_common_type<braid<L, R>>::type;
        std::array<common_t, detail::count_braid<braid<L, R>>::value> ar{ };
        br.for_each([it = ar.begin()](const common_t& value) mutable {
            *(it++) = value;
        });
        return ar;
    }

    template <class C> concept sv_braid = requires{ std::is_same_v<typename braid_common_type<C>::type, std::string_view>; };

    template <class L, class R> requires (sv_braid<braid<L, R>>) constexpr std::size_t sv_braid_size(const braid<L, R>& br) {
        return br.reduce([](std::size_t n, const std::string_view& sv){ return n + sv.size(); }, 0);
    }

    template <std::size_t Len, class L, class R> requires (sv_braid<braid<L, R>>) constexpr auto sv_braid_to_str(const braid<L, R>& br) {
        std::array<char, Len> ar{ };
        auto sv_array = braid_to_array(br);
        auto joined = sv_array | std::ranges::views::join;
        std::copy(joined.begin(), joined.end(), ar.begin());
        return ar;
    }

}

#define HT_BRAID_LITERAL(br) sv_braid_to_str<sv_braid_size(br)>(br)
