...
template <class L, class R = empty_strand> struct braid {
    ...
    constexpr auto as_tuple() {
        return reduce([]<class... T>(std::tuple<T...>&& t, const auto& value) {
...
template <class L, class R> requires(is_uniform_braid_v<braid<L, R>>)
...
template <class C> concept sv_braid =
        requires { std::is_same_v<typename braid_common_type<C>::type, std::string_view>; };

template <class L, class R> requires(sv_braid<braid<L, R>>)
...
template <std::size_t Len, class L, class R> requires(sv_braid<braid<L, R>>)
