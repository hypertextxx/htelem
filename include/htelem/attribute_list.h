...
struct identity_attr_type { };
template <static_string At> struct attribute_spec<At, identity_attr_type> {
    template <class U> constexpr auto operator=(U&& u) const { return std::forward<U>(u); }
};
...
constexpr auto initialize_aspect(attribute_list<El>& t, const std::tuple<AttrPtrs...>& ptrs,
