#ifndef HTELEM__UTILITIES_STATIC_STRING_H
#define HTELEM__UTILITIES_STATIC_STRING_H
...
template <std::size_t N> struct static_string {
    ...
    constexpr explicit(false) static_string(const char (&_data)[N + 1]) { std::copy_n(_data, N, data); }
    ...
    constexpr static_string<L + R>(const static_string<L>& lhs, const static_string<R>& rhs) {
